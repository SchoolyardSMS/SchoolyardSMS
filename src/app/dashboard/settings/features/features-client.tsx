  ...
  return (
    <div className="flex items-center justify-between">
      <div className="space-y-0.5">
        <label htmlFor={`feature-toggle-${name}`} className="text-base font-medium">{label}</label>
        <p className="text-xs text-slate-500">{description}</p>
      </div>
      <label htmlFor={`feature-toggle-${name}`} className="relative inline-flex items-center cursor-pointer">
        <input id={`feature-toggle-${name}`} type="checkbox" name={name} className="sr-only peer" defaultChecked={defaultChecked} aria-label={label} />
